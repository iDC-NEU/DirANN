...
namespace pipeann {
  ...
  class AbstractNeighbor {
   public:
    ...
    // for residual vector(put in query_buf->data_buf).
    virtual void compute_dists_with_residual(QueryBuffer<T> *query_buf, const uint32_t *ids, const uint64_t n_ids, const char *residual_data) {
      LOG(ERROR) << "compute_dists_with_residual is not implemented for " << get_name();
    };
