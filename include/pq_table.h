  ...
  class FixedChunkPQTable {
   ...
   public:
    ...
    void load_pq_pivots_new(std::basic_istream<char> &reader, size_t num_chunks, size_t offset) {
      ...
      dirann::load_bin_impl<_u64>(reader, file_offset_data_raw, nr, nc, offset);
      ...
      dirann::load_bin_impl<float>(reader, tables, nr, nc, file_offset_data[0] + offset);
      ...
      this->ndims = nc;
      dirann::load_bin_impl<float>(reader, centroid, nr, nc, file_offset_data[1] + offset);
      ...
      dirann::load_bin_impl<uint32_t>(reader, rearrangement, nr, nc, file_offset_data[2] + offset);
      if ((nr != this->ndims) || (nc != 1)) {
        LOG(ERROR) << "Rearrangement incorrect: row " << nr << ", col " << nc << " expecting " << this->ndims;
        crash();
      }

      dirann::load_bin_impl<uint32_t>(reader, chunk_offsets, nr, nc, file_offset_data[3] + offset);
    ...
    void post_load_pq_table() {
      ...
      // dirann::alloc_aligned((void **) &tables_T, 256 * ndims * sizeof(float), 64);
