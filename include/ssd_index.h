...
#include "neighbor.h"
#include "index.h"
#include "v3/unaligned_file_reader.h"
#include "v3/coord_buffer.h"
...
enum SearchMode { BEAM_SEARCH = 0, PAGE_SEARCH = 1, PIPE_SEARCH = 2, CORO_SEARCH = 3 , RERANK_SEARCH = 4};
...
namespace pipeann {
  ...
  class SSDIndex {
   public:
    ...
    SSDIndex(pipeann::Metric m, std::shared_ptr<AlignedFileReader> fileReader, std::shared_ptr<UnalignedFileReader> u_fileReader,
    ...
    inline unsigned *pmem_offset_to_node_nhood(const char *node_buf) {
      return (unsigned *) (node_buf + this->residual_data_len);
    }
    ...
    inline uint64_t pmem_u_loc_offset(uint64_t loc) {
      return 64 + loc * (pmem_node_len);
    }

    inline uint64_t pmem_u_loc_offset_nbr(uint64_t loc) {
      return 64 + loc * (pmem_node_len) + residual_data_len;
    }
    ...
    void init_metadata(const SSDIndexMetadata<T> &meta) {
      ...
      this->coord_len = data_dim * sizeof(T);
    ...
    void init_metadata(const SSDIndexMetadata<T> &meta, PmemIndexMetadata<T> &pmem_meta) {
      meta.print();
      pmem_meta.print();
      this->cur_id = this->num_points = this->init_num_pts = meta.npoints;
      this->data_dim = meta.data_dim;
      ...
      this->size_per_io = SECTOR_LEN * (nnodes_per_sector > 0 ? 1 : DIV_ROUND_UP(max_node_len, SECTOR_LEN));
      this->coord_len = data_dim * sizeof(T);
      this->range = pmem_meta.range;
      this->nhood_len = pmem_meta.nhood_len;
      this->residual_data_len = pmem_meta.residual_data_len;
      this->pmem_node_len = pmem_meta.max_node_len;
    ...
    // load compressed data, and obtains the handle to the disk-resident index and pmem index
    int load(const char *index_prefix, const char *pmem_index_prefix, uint32_t num_threads, bool new_index_format = true,
              bool use_page_search = false);
    ...
    size_t rerank_search(const T *query, const uint64_t k_search, const uint32_t mem_L, const uint64_t l_search,
                       TagT *res_tags, float *res_dists, const uint64_t beam_width, QueryStats *stats = nullptr,
                       tsl::robin_set<uint32_t> *deleted_nodes = nullptr, bool dyn_search_l = true);
   ...
   public:
    ...
    uint64_t nhood_len = 0;
    uint64_t residual_data_len = 0;
    uint64_t pmem_node_len = 0;
    uint64_t coord_len = 0;
    uint64_t size_per_io = 0;

    std::string _disk_index_file;

    std::shared_ptr<AlignedFileReader> reader = nullptr;
    std::shared_ptr<UnalignedFileReader> u_reader = nullptr;
    std::shared_ptr<CoordBuffer> coord_buffer = nullptr;
   ...
   public:
    ...
    int insert_in_place_pmemann(const T *point, const TagT &tag, tsl::robin_set<uint32_t> *deletion_set = nullptr);
    ...
    void do_rerank_search(const T *vec, uint32_t mem_L, uint32_t Lsize, const uint32_t beam_width,
                        std::vector<Neighbor> &expanded_nodes_info, tsl::robin_map<uint32_t, T *> *coord_map = nullptr,
                        T *coord_buf = nullptr, QueryStats *stats = nullptr,
                        tsl::robin_set<uint32_t> *exclude_nodes = nullptr, bool dyn_search_l = true,
                        std::vector<uint64_t> *passthrough_page_ref = nullptr);
   ...
   public:
    ...
    void consolidate_deletes_dirann(const std::string &in_path_prefix, const std::string &out_path_prefix,
    ...
    void merge_deletes_(const std::string &in_path_prefix, const std::string &out_path_prefix,
                        const std::vector<TagT> &deleted_nodes, const tsl::robin_set<TagT> &deleted_nodes_set,
                        uint32_t nthreads, const uint32_t &n_sampled_nbrs);
