...
#define READ_UNSIGNED(stream, val) stream.read((char *) &val, sizeof(unsigned))
  ...
  class SSDIndex {
   public:
    SSDIndex(dirann::Metric m, std::shared_ptr<AlignedFileReader> &fileReader, bool single_file_index,
    ...
    void init_query_buf(QueryBuffer<T> &buf) {
      ...
      dirann::alloc_aligned((void **) &buf.coord_scratch, coord_alloc_size, 256);
      dirann::alloc_aligned((void **) &buf.sector_scratch, MAX_N_SECTOR_READS * SECTOR_LEN, SECTOR_LEN);
      dirann::alloc_aligned((void **) &buf.aligned_pq_coord_scratch, 32768 * 32 * sizeof(_u8), 256);
      dirann::alloc_aligned((void **) &buf.aligned_pqtable_dist_scratch, 256 * MAX_PQ_CHUNKS * sizeof(float), 256);
      dirann::alloc_aligned((void **) &buf.aligned_dist_scratch, MAX_N_SECTOR_READS * sizeof(float), 256);
      dirann::alloc_aligned((void **) &buf.aligned_query_T, this->aligned_dim * sizeof(T), 8 * sizeof(T));
      dirann::alloc_aligned((void **) &buf.aligned_query_T_2, this->aligned_dim * sizeof(T), 8 * sizeof(T));
      dirann::alloc_aligned((void **) &buf.update_buf, (2 * MAX_N_EDGES + 1) * SECTOR_LEN,
    ...
    QueryBuffer<T> *pop_query_buf(const T *query) {
      ...
      if (likely(query != nullptr)) {
        if (data_is_normalized) {
          ...
          float norm = dirann::compute_l2_norm(query, this->data_dim);
   ...
   public:
    ...
    uint32_t delta_prune_neighbors_pq(std::vector<TriangleNeighbor> &pool, std::vector<uint32_t> &pruned_list,
   ...
   public:
    ...
    void verify_id2loc() {
      ...
      LOG(INFO) << "empty ID slot count: " << empty_id_count;
...
#ifdef USE_REVERSE_GRAPH
    void verify_in_graph() {
      auto p_reader = (LinuxAlignedFileReader *)this->reader.get();
      
      auto get_edges = [&](uint64_t id, bool print) {
        char * buf = nullptr;
        ...
        pread(p_reader->file_desc, buf, SECTOR_LEN, pid * SECTOR_LEN);

        char * node_buf = offset_to_loc(buf, loc);
        DiskNode<T> node(id, offset_to_node_coords(node_buf), offset_to_node_nhood(node_buf));
        std::vector<uint32_t> out_nbrs(node.nnbrs);
        if (node.nnbrs > range) {
          LOG(ERROR) << id << ".nnbrs " << node.nnbrs << " out of range\n";
          exit(-1);
        }
        for (uint32_t i = 0; i < node.nnbrs; i++) {
          out_nbrs[i] = node.nbrs[i];
          if (out_nbrs[i] >= cur_id.load() || out_nbrs[i] < 0) {
            LOG(ERROR) << out_nbrs[i] << " nbrs out of range\n";
            exit(-1);
          }
        }
        if (print) {
      ...
      // {
      //   std::cerr << "################################\n";
      //   #pragma omp parallel for num_threads(32) schedule(dynamic, 1024)
      //       for (uint64_t loc = 0; loc < cur_loc.load(); loc ++){
      //         uint32_t id2 = loc2id(loc);
      //         if (id2 == kInvalidID) {
      //           continue;
      //         }
      //         std::vector<int> in_nbrs;
      //         int id = 458066;
      //         disk_in_graph->get_edges(id2, in_nbrs);
      //         if (std::find(in_nbrs.begin(), in_nbrs.end(), id)!= in_nbrs.end()) {
      //           #pragma omp critical
      //           std::cerr << id << "<-" << id2 << "\n";
      //         }
      //       }
      ...
      std::mutex mtx;
      {
        ...
        for (uint64_t loc = 0; loc < cur_loc.load(); loc ++){
          ...
          cnt ++;
          auto nbrs = get_edges(id, false);
          int flag = 0;
          for (auto &nbr : nbrs) {
            if (nbr != this->medoids[0]) {
              // if (disk_in_graph->del_edge(nbr, id)) {
              std::vector<int> in_nbrs;
              disk_in_graph->get_edges(nbr, in_nbrs);
              if (std::find(in_nbrs.begin(), in_nbrs.end(), id)== in_nbrs.end()) {
                flag = 1;
                #pragma omp critical
                std::cerr << id << "<-!" << nbr << "\n";
              }
              //   // std::cerr << "node " << nbr << " has " << in_nbrs.size() << " in reverse_edges\n";
              //   // for (int i = 0; i < in_nbrs.size(); i++) {
              //   //   std::cerr << in_nbrs[i] << " ";
              //   // }
              //   // std::cerr << "\n";
              // };
              // disk_in_graph->add_edge(nbr, id);
            }
          }
          
          std::lock_guard<std::mutex> lock(mtx);
          if (flag || id2tag(id) == 609270) {
          // if (flag ) {
            
            std::cerr << "fuck tag is " << id2tag(id) << "\n";
            // std::cerr << "BAD id is " << id << "\n";
            std::cerr << "size " << nbrs.size() << " id " << id << ", nbrs is :\n";
            for (auto &nbr : nbrs){
              std::cerr << id << "->" << nbr << "\n";
            }
            std::cerr << "&&&&&&&&&&&&&&&&&&&&&&, rever nbr is\n";
            #pragma omp parallel for num_threads(32) schedule(dynamic, 1024)
            for (uint64_t loc = 0; loc < cur_loc.load(); loc ++){
              uint32_t id2 = loc2id(loc);
              if (id2 == kInvalidID) {
            ...
            // std::cerr << "***************\n";
            // std::vector<int> in_nbrs;
            // disk_in_graph->get_edges(id, in_nbrs);
            // for (int i = 0; i < in_nbrs.size(); i++) {
            //   #pragma omp critical
            //   std::cerr << in_nbrs[i] << "->" << id << "\n";
            // }
            // std::cerr << "\n&&&&&&&&&&&&&&&&&&\n";
            // #pragma omp parallel for num_threads(32) schedule(dynamic, 1024)
            // for (uint64_t loc = 0; loc < cur_loc.load(); loc ++){
    ...
    void trigger_deletion_in_place2(const tsl::robin_set<TagT> &deleted_nodes_set, uint32_t nthreads,
                                          const uint32_t &n_sampled_nbrs);

    void trigger_deletion_in_place3(const tsl::robin_set<TagT> &deleted_nodes_set, uint32_t nthreads,
                                          const uint32_t &n_sampled_nbrs);

    void trigger_deletion_in_place4(const tsl::robin_set<TagT> &deleted_nodes_set, uint32_t nthreads,
                                            const uint32_t &n_sampled_nbrs);

    void trigger_deletion_in_place5(const tsl::robin_set<TagT> &deleted_nodes_set, uint32_t nthreads,
                                          const uint32_t &n_sampled_nbrs);
   ...
   private:
    ...
    // #ifdef REVERSE_GRAPH
      ...
      livegraph::BlockGraph *disk_in_graph;
    #else 
      DynGraphDisk::BlockGraph *disk_in_graph;
    #endif
    // #endif
  };
}  // namespace dirann
