...
public:
  ...
  void add_edge(Block* block, edge_t dst) {
    ...
    if (block->get_last_value_id() + 1 < block->get_max_edge_num()) {
      int i = block->get_last_value_id() + 1;
      nbrs[i] = dst;
      block->set_nbr_num(block->get_nbr_num() + 1);
      if (i > block->get_last_value_id()) {
        block->set_last_value_id_(i);
      }
    ...
    for (int i = 0; i < block->get_max_edge_num(); ++i) {
      if (is_deleted(nbrs[i]) == true) {
        nbrs[i] = dst;
        block->set_nbr_num(block->get_nbr_num() + 1);
        if (i > block->get_last_value_id()) {
          block->set_last_value_id_(i);
    ...
    for (int i = 0; i < block->get_max_edge_num(); ++i) {
      if (nbrs[i] == dst) {
        ...
        if (dst == block->get_min_value()) {
          ...
          for (int j = 0; j < block->get_max_edge_num(); ++j) {
            if (is_deleted(nbrs[j]) == false) {
...
class BlockGraph {
public:
  BlockGraph(std::string path, size_t max_vertex, int edge_num_per_block, int merge_block_num,
             bool load_old_data, size_t capacity = 1ul << 30) 
    : blockmanager(path, load_old_data, capacity),
      writelock_(max_vertex),
      edge_num_per_block(edge_num_per_block),
      graph_(max_vertex, nullptr),
      merge_block_num_(merge_block_num),
      filepath_(path),
      load_old_data_(load_old_data),
      max_vertex_(max_vertex) {
    std::cout << "LiveGraph BlockGraph constructed." << std::endl;
  ...
  void save_meta() {
    ...
    outfile.write(reinterpret_cast<const char*>(&edge_num_per_block), sizeof(edge_num_per_block));
    ...
    std::cout << "  edge_num_per_block_:" << edge_num_per_block << std::endl;
  ...
  void load_meta() {
    ...
    infile.read(reinterpret_cast<char*>(&edge_num_per_block), sizeof(edge_num_per_block));
  ...
  void add_edge(vertex_t src, edge_t dst) {
    ...
    if (block == nullptr) {
      ...
      int block_size = sizeof(Block::Head) + sizeof(edge_t) * edge_num_per_block;
      char* new_pointer = blockmanager.alloc(block_size);

      if (graph_[src] != nullptr) {
        delete graph_[src];
      }
      graph_[src] = new Block(new_pointer, edge_num_per_block, -1);
      block = graph_[src];
    }
    ...
    if (block->get_max_edge_num() <= block->get_nbr_num()) { 
      ...
      if (new_pointer == nullptr) {
        ...
        graph_[src] = new Block(new_pointer, edge_num_per_block, 
  ...
  void sample_degree() {
    ...
    degree_count.reserve(sample_num);
    for (vertex_t key = 0; key < max_vertex_; key++) {
      ...
      if (degree_count.size() >= sample_num) {
        break;
      }
    }
