...
#define MAX_N_SECTOR_READS 1024ULL
...
namespace pipeann {
  ...
  struct PmemNode {
    ...
    // id : id of node
    // sector_buf : sector buf containing `id` data
    PmemNode(uint32_t id, uint8_t *codes, uint32_t *nhood);
  };
  ...
  template<class T>
  struct PmemIndexMetadata {
    
    uint64_t npoints;  // size.
    uint64_t max_node_len;
    uint64_t range;     // maximum out-degree.
    uint64_t data_dim;
    uint64_t residual_bits; // number of bits for residual vector.
    uint64_t nhood_len;
    uint64_t residual_data_len;

    PmemIndexMetadata() {
    }

    void print() const {
      LOG(INFO) << "Npoints: " << npoints << " Data dim: " << data_dim << " Range: " << range;
      LOG(INFO) << "Max node len: " << max_node_len << " Residual bits: " << residual_bits;
      LOG(INFO) << "Nhood len: " << nhood_len << " Residual data len: " << residual_data_len;
    }

    void load_from_pmem_index(const std::string &filename) {
      if (std::filesystem::exists(filename) == false) {
        LOG(ERROR) << "File " << filename << " does not exist.";
        exit(-1);
      }
      std::ifstream in(filename, std::ios::binary);
      load_from_pmem_index(in);
      in.close();
    }

    void load_from_pmem_index(std::ifstream &in) {
      LOG(INFO) << "Loading metadata from pmem index.";
      in.read((char *) &npoints, sizeof(uint64_t));
      in.read((char *) &max_node_len, sizeof(uint64_t));
      in.read((char *) &range, sizeof(uint64_t));
      in.read((char *) &data_dim, sizeof(uint64_t));
      in.read((char *) &residual_bits, sizeof(uint64_t));

      this->nhood_len = (range + 1) * sizeof(unsigned);
      this->residual_data_len = residual_bits ? DIV_ROUND_UP(residual_bits * data_dim, 8) + 2 * sizeof(float) : 0;
    }

    void save_to_pmem_index(const std::string &filename) {
      std::ofstream out(filename, std::ios::in | std::ios::out | std::ios::binary);
      save_to_pmem_index(out);
      out.close();
    }

    void save_to_pmem_index(std::ofstream &out) {
      out.write((char *) &npoints, sizeof(uint64_t));
      out.write((char *) &max_node_len, sizeof(uint64_t));
      out.write((char *) &range, sizeof(uint64_t));
      out.write((char *) &data_dim, sizeof(uint64_t));
      out.write((char *) &residual_bits, sizeof(uint64_t));
    }
  };
};  // namespace pipeann
