  ...
  class Index {
   public:
    ...
    size_t search_with_tags(const T *query, const uint64_t K, const unsigned L, TagT *tags, float *distances,
                            std::vector<T *> &res_vectors, QueryStats *stats = nullptr);
    ...
    inline void get_coord(size_t id, T* buffer) const;

    uint8_t get_target_numa_node(const T *query_coords, T *coord_scratch);
    ...
    void full_pruning(Parameters &parameters);
   ...
   public:
    ...
    std::pair<uint32_t, uint32_t>  get_expanded_nodes(const size_t node, const unsigned Lindex, std::vector<unsigned> init_ids,
                            std::vector<Neighbor> &expanded_nodes_info, tsl::robin_set<unsigned> &expanded_nodes_ids);

    std::pair<int, size_t>  inter_insert(unsigned n, std::vector<unsigned> &pruned_list, const Parameters &parameter);

    int prune_neighbors(const unsigned location, std::vector<Neighbor> &pool, const Parameters &parameter,
                         std::vector<unsigned> &pruned_list);

    void delta_prune_neighbors(const Parameters &parameter, std::vector<uint32_t> &nhood, uint32_t center_id, uint32_t target_id);

    int occlude_list(std::vector<Neighbor> &pool, const float alpha, const unsigned degree, const unsigned maxc,
                      std::vector<Neighbor> &result);

    int occlude_list(std::vector<Neighbor> &pool, const float alpha, const unsigned degree, const unsigned maxc,
                      std::vector<Neighbor> &result, std::vector<float> &occlude_factor, bool skip = false);
    ...
    size_t consolidate_deletes_dirann(const Parameters &parameters);
    uint32_t find_valid_ep_after_delete() const;
    void repair_connectivity_after_delete_dirann(const Parameters &parameters);
    void find_anchor_candidates_by_search(uint32_t u,
                                          const std::vector<uint8_t> &visited,
                                          const Parameters &parameters,
                                          size_t c,
                                          std::vector<Neighbor> &anchors);

    size_t consolidate_deletes_wolverine(const Parameters &parameters);
    size_t consolidate_deletes_wolverine_old(const Parameters &parameters);

    size_t consolidate_deletes_greator(const Parameters &parameters);

    size_t consolidate_deletes_ip_diskann(const Parameters &parameters);

    size_t consolidate_deletes_hnsw(const Parameters &parameters);
   ...
   public:
    ...
    // numa-aware data pointers
    T * _numa_data_ptrs[4] = {nullptr, nullptr, nullptr, nullptr};
    uint8_t _numa_nodes;
    std::vector<size_t> _numa_cluster_map; // TODO(yq): move to pmem
    std::vector<uint32_t> _numa_cluster_ids;
    std::vector<size_t> _numa_mmap_lens;
