...
void check_and_gen(char *gt_path, uint64_t index_npts, uint64_t tot_npts, uint64_t batch_npts, uint64_t n_batches, uint64_t target_topk,
  ...
  if (std::filesystem::exists(gt_path)) {
    LOG(INFO) << "GT file " << gt_path << " exists.";
  } else {
    LOG(FATAL) << "GT file " << gt_path << " does not exist.";
  }
  ...
  for (uint64_t step = 0; step < n_steps; ++step) {
    ...
    if (ed > tot_npts) {
      LOG(FATAL) << "Range end " << ed << " exceeds total number of points " << tot_npts
                << " at step " << step;
      break;
    }

    if (real_st >= tot_npts) {
      LOG(FATAL) << "Range start " << real_st << " exceeds total number of points " << tot_npts
                << " at step " << step;
      break;
    }
  ...
  if (uint64_t(success_cnt) != n_steps) {
    LOG(FATAL) << "Some steps failed, please check the gt file.";
  }
}
...
int main(int argc, char **argv) {
  ...
  uint64_t n_batches = std::stoull(argv[arg_no++]);
  ...
  check_and_gen(gt_path, index_npts, npts, batch_npts, n_batches, target_topk, target_dir, insert_only);
