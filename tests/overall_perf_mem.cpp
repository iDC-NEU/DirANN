...
void sync_search_kernel(T *query, size_t query_num, size_t query_dim, const int recall_at, uint64_t L,
  ...
  if (calRecall) {
    ...
    pipeann::load_truthset(truthset_file, gt_ids, gt_dists, gt_num, gt_dim, &tags);
  }
...
void update(const std::string &data_bin, const unsigned L_disk, const unsigned R_disk, const float alpha_disk, int step,
  ...
  // pipeann::Metric metric = pipeann::Metric::COSINE;
  std::cout << "Set metric=" << metric << std::endl;

  pipeann::Index<T, TagT> sync_index(metric, dim, kMaxPoints, true, false, true);
  sync_index.load_from_disk_index(save_path);
  ...
  params.set(sync_index.range, L_disk, 384, 1.2);
  ...
  //   LOG(INFO) << "Perform a full pruning.";
  //   sync_index.full_pruning(params);
  // }
  ...
  uint64_t vecs_per_step = index_npts * 0.1;
  for (int i = 0; i < batch; i++) {
    ...
    if (i == batch
      ...
      do {
        merge_status = merge_future.wait_for(std::chrono::seconds(2));
        // double dummy = 0;
        // sync_search_kernel(query, query_num, query_dim, recall_at, Lsearch[0], sync_index, currentFileName,
        //                    false, false, dummy);
  ...
  #ifdef CALC_RECALL_AFTER_UPDATE
  {
    Lsearch.clear();
  ...
  #ifdef SAVE_INDEX_AFTER_UPDATE
    std::string save_name = std::string(save_path + "_after_step" + std::to_string(step) + "_batchsize" + std::to_string(vecs_per_step)).c_str();
    std::string sys_name = "DiskANN";
    save_name += "_" + sys_name;
    sync_index.save(save_name.c_str());
    std::cout << "SAVE_INDEX_AFTER_UPDATE: " << save_name << std::endl;
  #endif
