...
void sync_search_kernel(T *query, size_t query_num, size_t query_aligned_dim, const int recall_at, _u64 L,
                        dirann::Index<T, TagT> &sync_index, std::string &truthset_file, bool merged, bool calRecall,
                        double &disk_io) {
...
template<typename T, typename TagT>
void merge_kernel(dirann::Index<T, TagT> &sync_index, std::string &save_path) {
  sync_index.consolidate(params);
}

template<typename T, typename TagT>
void deletion_kernel(T *data_load, dirann::Index<T, TagT> &sync_index, std::vector<TagT> &delete_vec,
                     size_t aligned_dim) {
  dirann::Timer timer;
...
template<typename T, typename TagT>
void insertion_kernel(T *data_load, dirann::Index<T, TagT> &sync_index, std::vector<TagT> &insert_vec,
                      size_t aligned_dim) {
  dirann::Timer timer;
...
void update(const std::string &data_bin, const unsigned L_disk, const unsigned R_disk, const float alpha_disk, int step,
  ...
  dirann::Index<T, TagT> sync_index(metric, dim, kMaxPoints, true, false, true);
  ...
  //   std::vector<dirann::Neighbor> best_L_nodes, expanded_nodes_info;
