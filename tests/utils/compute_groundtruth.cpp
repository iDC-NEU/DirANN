...
void exact_knn2(const size_t dim, const size_t k,
...
void exact_knn(const size_t dim, const size_t k,
  ...
  std::cout << "Going to compute " << k << " NNs for " << nqueries << " queries over " << npoints << " points in "
            << dim << " dimensions using l2" << std::endl;
  ...
  for (uint64_t b = 0; b < DIV_ROUND_UP(nqueries, q_batch_size); ++b) {
    ...
    for (int64_t qi = 0; qi < q_e - q_b; qi++) {
        const float *qptr = queries_in + (q_b + qi) * dim;

        for (size_t p = 0; p < npoints; p++) {
            const float *pptr = points_in + p * dim;

            float dist = 0.0f;
            for (size_t d = 0; d < dim; d++) {
                float diff = qptr[d] - pptr[d];
                dist += diff * diff;
            }

            dist_matrix[qi * npoints + p] = dist;
        }
    }
    std::cout << "Computed distances for queries: [" << q_b << "," << q_e << ")" << std::endl;
    if (q_b == 0 && npoints > 0) {
        ...
        float manual = 0.0f;
        for (size_t d = 0; d < dim; d++) {
            float diff = queries_in[test_q * dim + d] - points_in[test_p * dim + d];
            manual += diff * diff;
        }

        float bulk = dist_matrix[test_q * npoints + test_p];

        std::cout << "manual dist(q0,p0) = " << manual << std::endl;
        std::cout << "bulk   dist(q0,p0) = " << bulk << std::endl;
    }
...
int aux_main(int argc, char **argv) {
  ...
  for (int p = 0; p < num_parts; p++) {
    ...
    size_t part_k = std::min(k, npoints);
    int *closest_points_part = new int[nqueries * part_k];
    float *dist_closest_points_part = new float[nqueries * part_k];
