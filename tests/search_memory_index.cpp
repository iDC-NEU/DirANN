...
int search_memory_index(int argc, char **argv) {
  ...
  if (file_exists(truthset_bin)) {
    ...
    if (gt_tags == nullptr) {
      // Some truthsets store only ids (or ids+dists) without explicit tags.
      // For identity-tag workloads, fallback to ids as tags to avoid null dereference.
      gt_tags = gt_ids;
      LOG(INFO) << "Truthset has no explicit tags; falling back to ids-as-tags for recall calculation.";
    }
  ...
  std::cout << std::setw(4) << "Ls" << std::setw(12) << "QPS " << std::setw(18) << "Mean Latency (ms)" << std::setw(15)
            << "99.9 Latency" << std::setw(12) << "Mean Hops" << std::setw(12) << "Mean Cmps" << std::setw(12)
            << recall_string << std::endl;
  std::cout << "==============================================================="
               "==============="
            << std::endl;
...
#pragma omp parallel for schedule(dynamic, 1) num_threads(num_threads)
    ...
    float mean_hops =
        (float) pipeann::get_mean_stats(query_stats.data(), query_num,
    ...
    std::cout << std::setw(4) << L << std::setw(12) << qps << std::setw(18) << (float) mean_latency << std::setw(15)
              << (float) latency_stats[(uint64_t) (0.999 * (double) query_num)] << std::setw(12) << mean_hops
              << std::setw(12) << mean_cmps << std::setw(12) << recall << std::endl;
  }
