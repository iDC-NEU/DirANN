...
int search_memory_index(int argc, char **argv) {
  ...
  dirann::Metric m = (distance_metric == "cosine" ? dirann::Metric::COSINE : dirann::Metric::L2);

  if (distance_metric != "l2" && m == dirann::Metric::L2) {
  ...
  std::cout << "Running search on type " << typeid(T).name() << " max_points: " << max_points
            ...
            << " similarity metric: " << (m == dirann::Metric::COSINE ? "cosine" : "l2") << " first L: " << Lvec[0]
            << std::endl;

  dirann::load_aligned_bin<T>(query_bin, query, query_num, query_dim, query_aligned_dim);

  if (file_exists(truthset_bin)) {
    dirann::load_truthset(truthset_bin, gt_ids, gt_dists, gt_num, gt_dim, &gt_tags);
    if (gt_num != query_num) {
  ...
  dirann::Index<T, uint32_t> index(m, query_dim, max_points, dynamic_index, single_index_file, dynamic_index);
...
#pragma omp parallel for schedule(dynamic, 1) num_threads(num_threads)
    ...
    if (calc_recall_flag)
      ...
      recall = (float) dirann::calculate_recall((_u32) query_num, gt_tags, gt_dists, (_u32) gt_dim,
