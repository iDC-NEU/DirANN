...
int search_disk_index(int argc, char **argv) {
  ...
  // dirann::load_aligned_bin<T>(query_bin, query, query_num, query_dim, query_aligned_dim);
  ...
  dirann::Index<T> _pFlashIndex(m, query_dim, (uint64_t) 1e8, false, false, false);
...
#pragma omp parallel for schedule(dynamic, 1)
    ...
    float mean_cpuus = (float) dirann::get_mean_stats(stats, query_num,
                                                       [](const dirann::QueryStats &stats) { return stats.cpu_us; });

    float mean_cpu1us = (float) dirann::get_mean_stats(stats, query_num,
                                                        [](const dirann::QueryStats &stats) { return stats.cpu_us1; });

    float mean_cpu2us = (float) dirann::get_mean_stats(stats, query_num,
                                                        [](const dirann::QueryStats &stats) { return stats.cpu_us2; });

    float mean_ious =
        (float) dirann::get_mean_stats(stats, query_num, [](const dirann::QueryStats &stats) { return stats.io_us; });

    float mean_io1us = (float) dirann::get_mean_stats(stats, query_num,
                                                       [](const dirann::QueryStats &stats) { return stats.io_us1; });
    delete[] stats;
    ...
    if (calc_recall_flag) {
      ...
      recall = (float) dirann::calculate_recall((_u32) query_num, gt_ids, gt_dists, (_u32) gt_dim,
  ...
  //   dirann::save_bin<_u32>(cur_result_path, query_result_ids[test_id].data(), query_num, recall_at);

  //   cur_result_path = result_output_prefix + "_" + std::to_string(L) + "_tags_uint32.bin";
  //   dirann::save_bin<_u32>(cur_result_path, query_result_tags[test_id].data(), query_num, recall_at);
  //   cur_result_path = result_output_prefix + "_" + std::to_string(L) + "_dists_float.bin";
  //   dirann::save_bin<float>(cur_result_path, query_result_dists[test_id++].data(), query_num, recall_at);
  // }

  // dirann::aligned_free(query);
  // if (warmup != nullptr)
  //   dirann::aligned_free(warmup);
  // delete[] gt_ids;
  // delete[] gt_dists;
  return 0;
}
