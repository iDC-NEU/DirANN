...
int search_disk_index(int argc, char **argv) {
  ...
  int _numa_nodes = 2;
  LOG(INFO) << "Num NUMA nodes: " << _numa_nodes << ", threads per node: " << (num_threads / _numa_nodes);
  NumaThreadPool thread_pool(_numa_nodes, num_threads / _numa_nodes);
#else
  LOG(INFO) << "Num threads: " << num_threads;
  omp_set_num_threads(num_threads);
#endif
  pipeann::Index<T> _pFlashIndex(m, query_dim, (uint64_t) 1e6, false, false, false);
  ...
  for (uint32_t test_id = 0; test_id < Lvec.size(); test_id++) {
    ...
    alignas(4096) T router_coord_scratch[1024]; 

    for (int64_t i = 0; i < (int64_t)query_num; i++) {
        
        uint8_t min_nid = _pFlashIndex.get_target_numa_node(query + (i * query_dim), router_coord_scratch);

        
        thread_pool.submit(min_nid, [=, &stats, &_pFlashIndex, &query_result_tags_32, &query_result_dists]() {
            auto s1 = std::chrono::high_resolution_clock::now();
            
            
            _pFlashIndex.search(
                query + (i * query_dim), recall_at, L, 
        ...
        // , NumaThreadPool::OverflowStrategy::SPILLOVER
        );
    }
...
#pragma omp parallel for schedule(dynamic, 1)
    ...
    float mean_io1us = (float) pipeann::get_mean_stats(stats, query_num,
                                                       [](const pipeann::QueryStats &stats) { return stats.numa_cpu_us; });
    delete[] stats;
