...
template<typename T, typename TagT>
void merge_kernel(dirann::DynamicSSDIndex<T, TagT> &sync_index) {
...
void update(const std::string &data_bin, const unsigned L_disk, int vecs_per_step, int num_steps,
            ...
            const uint32_t search_beam_width, const uint32_t search_mem_L, dirann::Distance<T> *dist_cmp) {
  ...
  dirann::load_bin<T>(query_file, query, query_num, query_dim);

  dim = query_dim;
  dirann::Metric metric = dirann::Metric::L2;
  dirann::DynamicSSDIndex<T, TagT> sync_index(paras, index_prefix, index_prefix + "_merge", dist_cmp, metric,
