...
#pragma omp parallel for num_threads(NUM_SEARCH_THREADS) schedule(dynamic)
  for (int64_t i = 0; i < (int64_t) query_num; i++) {
    sync_index.search(query + i * query_dim, recall_at, mem_L, L, beam_width, query_result_tags + i * recall_at,
                      query_result_dists + i * recall_at, stats + i, false);
...
void update(const std::string &data_bin, const unsigned L_disk, int vecs_per_step, int num_steps,
            const std::string &index_prefix, const std::string &pmem_index_prefix, 
...
#ifdef PMEMANN
  pipeann::DynamicSSDIndex<T, TagT> sync_index(paras, index_prefix, index_prefix + "_merge", pmem_index_prefix, pmem_index_prefix + "_merge", dist_cmp, metric,
...
int main(int argc, char **argv) {
  if (argc < 19) {
    LOG(INFO) << "Correct usage: " << argv[0] << " <type[int8/uint8/float]> <data_bin> <L_disk>"
              ...
              << " <index_prefix> <pmem_index_prefix> <query_file> <truthset_prefix> <truthset_l_offset> <recall@>"
  ...
  std::string pmem_index_prefix(argv[arg_no++]);
  ...
  if (std::string(argv[1]) == std::string("int8")) {
    ...
    update<int8_t, unsigned>(data_bin, L_disk, vecs_per_step, num_steps, index_prefix, pmem_index_prefix, query_file, truthset,
                             truthset_l_offset, recall_at, Lsearch, beam_width, search_beam_width, search_mem_L,
                             &dist_cmp);
  } else if (std::string(argv[1]) == std::string("uint8")) {
    pipeann::DistanceL2UInt8 dist_cmp;
    update<uint8_t, unsigned>(data_bin, L_disk, vecs_per_step, num_steps, index_prefix, pmem_index_prefix, query_file, truthset,
                              truthset_l_offset, recall_at, Lsearch, beam_width, search_beam_width, search_mem_L,
                              &dist_cmp);
  } else if (std::string(argv[1]) == std::string("float")) {
    pipeann::DistanceL2 dist_cmp;
    update<float, unsigned>(data_bin, L_disk, vecs_per_step, num_steps, index_prefix, pmem_index_prefix, query_file, truthset,
                            truthset_l_offset, recall_at, Lsearch, beam_width, search_beam_width, search_mem_L,
                            &dist_cmp);
  } else
