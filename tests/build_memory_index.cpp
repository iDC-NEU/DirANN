...
int build_in_memory_index(const std::string &data_path, const std::string &tags_file, const unsigned R,
                          ...
                          bool dynamic_index, bool single_file_index, dirann::Metric distMetric) {
  ...
  dirann::get_bin_metadata(data_path, data_num, data_dim);
  std::cout << "Building in-memory index with parameters: data_file: " << data_path << "tags file: " << tags_file
            ...
            << " #threads: " << num_threads
            << ", using distance metric: " << (distMetric == dirann::Metric::COSINE ? "cosine " : "l2 ");

  typedef uint32_t TagT;

  dirann::Index<T, TagT> index(distMetric, data_dim, data_num, dynamic_index, single_file_index,
...
int main(int argc, char **argv) {
  ...
  enum dirann::Metric distMetric = dist_metric_str == "cosine"
                                        ? dirann::Metric::COSINE
                                        : dirann::Metric::L2;  // set to l2 even if something else is chosen

  if (dist_metric_str != "l2" && distMetric == dirann::Metric::L2) {
