...
int search_disk_index(int argc, char **argv) {
...
#ifdef PMEMANN
  std::string pmem_prefix_path(argv[index++]);
#endif
  ...
  //   nbr_handler = new pipeann::RaBitQNeighbor<T>();
    ...
    nbr_handler = new pipeann::LVQNeighbor<T, PRIMARY_BITS, RESIDUAL_BITS>();  
  } else {
...
#ifdef PMEMANN
  if (nbr_type != "lvq") {
    LOG(ERROR) << "Only LVQ nbr type is supported for PMEMANN";
    exit(1);
  }
  u_reader.reset(new UnalignedFileReader());
  _pFlashIndex.reset(new pipeann::SSDIndex<T>(m, reader, u_reader, nbr_handler, tags_flag));
  int res = _pFlashIndex->load(index_prefix_path.c_str(), pmem_prefix_path.c_str(), num_threads, true, use_page_search);
#else
...
#pragma omp parallel for schedule(dynamic, 1)
...
#ifdef PMEMANN
    float mean_pmem_nhood_io_us = (float) pipeann::get_mean_stats(stats, query_num,
      [](const pipeann::QueryStats &stats) { return stats.pmem_io_us; });
    
    float mean_rerank_us = (float) pipeann::get_mean_stats(stats, query_num,
      [](const pipeann::QueryStats &stats) { return stats.rerank_us; });
    
    float mean_rerank_io_us = (float) pipeann::get_mean_stats(stats, query_num,
      [](const pipeann::QueryStats &stats) { return stats.rerank_io_us; });
    
    float mean_primary_4bit_cpu_us = (float) pipeann::get_mean_stats(stats, query_num,
      [](const pipeann::QueryStats &stats) { return stats.primary_4bit_cpu_us; });
    
    float mean_residual_8bit_io_cpu_us = (float) pipeann::get_mean_stats(stats, query_num,
      [](const pipeann::QueryStats &stats) { return stats.residual_8bit_io_cpu_us; });
#endif
    ...
    if (output) {
      ...
      std::cout << std::setw(6) << L << std::setw(12) << beamwidth << std::setw(12) << qps << std::setw(12)
                ...
                << mean_ios
#ifdef PMEMANN
                << std::setw(12) << mean_primary_4bit_cpu_us << std::setw(12) << mean_residual_8bit_io_cpu_us 
                << std::setw(12) << mean_pmem_nhood_io_us << std::setw(12) << mean_rerank_io_us
                << std::setw(12) << mean_rerank_us
#endif
              ;
      if (calc_recall_flag) {
...
#ifdef PMEMANN
            << std::setw(12) << " PrimaryCpuUs" << std::setw(12) << " ResidualCpuUs"
            << std::setw(12) << "PmemIoUs"
            << std::setw(12) << "RerankIoUs"  << std::setw(12) << "Rerank us"
#endif
            << std::setw(12);
  if (calc_recall_flag) {
...
int main(int argc, char **argv) {
  if (argc < 13) {
    ...
    std::cout << "Usage: " << argv[0]
              << " <index_type (float/int8/uint8)>  <index_prefix_path> <pmem_index_prefix_path>"
                 " <num_threads>  <pipeline width> "
                 " <query_file.bin>  <truthset.bin (use \"null\" for none)> "
                 " <K> <similarity (cosine/l2)> <nbr_type (pq/rabitq/lvq)>"
