...
namespace dirann {
  ...
  void SSDIndex<T, TagT>::trigger_deletion_in_place3(const tsl::robin_set<TagT> &deleted_nodes_set, uint32_t nthreads,
                                        const uint32_t & _) {
    if (nthreads == 0) {
    ...
    tsl::robin_set<uint32_t> affected_ids;
...
#define sqr(x) ((x)*(x))
    ...
    auto consolidate_deletes = [&](DiskNode<T> &node, uint8_t *thread_pq_buf) {
      ...
      for (uint32_t i = 0; i < node.nnbrs; ++i) {
        ...
        //   const auto &[y_ids, yz_dists] = deleted_nhoods.find(z_id);
        //   // const auto &yz_dists = deleted_nhoods_dist.find(z_id);
        ...
        //     // if (xy_dists[j] > yz_dists[j]){
        //     if (sqr(yz_dists[j]) + sqr(xy_dists[j]) <= sqr(xz_dists[i])) continue;
