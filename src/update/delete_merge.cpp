...
#define SECTORS_PER_MERGE 65536
  ...
  void SSDIndex<T, TagT>::merge_deletes(const std::string &in_path_prefix, const std::string &out_path_prefix,
    ...
    // uint32_t populate_nthreads = nthreads; 
...
#pragma omp parallel for num_threads(populate_nthreads) schedule(dynamic, 1024)
    ...
    auto write_back = [&]() {
      ...
      if (gs != nullptr) {
        ...
        gs->delete_ios += write_reqs[0].len;
      }
      // delete_io_ll.fetch_add(delete_io_timer.elapsed());
      wb_id += id_delta;
...
#pragma omp parallel for num_threads(nthreads) schedule(dynamic, 1024)
      for (uint64_t loc = loc_st; loc < loc_ed; ++loc) {
        ...
        //   continue;
        // }
        // freshdiskann总删除时间1.989698 s
        // 从这里continue, 1.45693 s
...
#pragma omp parallel for num_threads(nthreads)
  ...
  void SSDIndex<T, TagT>::write_metadata_and_pq(const std::string &in_path_prefix, const std::string &out_path_prefix,
    ...
    Timer timer;
    std::string disk_index_out = out_path_prefix + "_disk.index";
    dirann::save_bin<uint64_t>(disk_index_out, output_metadata.data(), output_metadata.size(), 1, 0);
    ...
    timer.reset();
    dirann::save_bin<TagT>(out_path_prefix + "_disk.index.tags", new_tags->data(), new_npoints, 1, 0);

    // write PQ pivots.
    std::string pq_out = out_path_prefix + "_pq_compressed.bin";
    dirann::save_bin<uint8_t>(pq_out, this->data.data(), new_npoints, n_chunks);
    ...
    if(gs != nullptr) {
      ...
      gs->delete_ios += new_medoid * this->pq_table.n_chunks;
    }
  }
