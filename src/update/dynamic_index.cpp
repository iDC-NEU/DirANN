...
namespace pipeann {
  ...
  template<typename T, typename TagT>
  DynamicSSDIndex<T, TagT>::DynamicSSDIndex(Parameters &parameters, 
                                            const std::string disk_prefix_in, const std::string disk_prefix_out,
                                            const std::string pmem_prefix_in, const std::string pmem_prefix_out,
                                            Distance<T> *dist,
                                            pipeann::Metric dist_metric, int search_mode, bool use_mem_index) {
    // check if file exists.
    if (!std::filesystem::exists(disk_prefix_in + "_disk.index")) {
    ...
    // check if pmem file exists.
    if (!std::filesystem::exists(pmem_prefix_in + "_pmem.index")) {
      LOG(ERROR) << "Pmem index file does not exist: " << pmem_prefix_in << "_pmem.index";
      exit(-1);
    }
    if (use_mem_index && !std::filesystem::exists(disk_prefix_in + "_mem.index")) {
      LOG(ERROR) << "In-memory index file does not exist: " << disk_prefix_in << "_mem.index";
    ...
    _disk_index_prefix_in = disk_prefix_in;
    _disk_index_prefix_out = disk_prefix_out;
    _pmem_index_prefix_in = pmem_prefix_in;
    _pmem_index_prefix_out = pmem_prefix_out;
    _dist_comp = dist;

    reader.reset(new LinuxAlignedFileReader());

#ifdef PMEMANN
    AbstractNeighbor<T> *nbr_handler = new LVQNeighbor<T, PRIMARY_BITS, RESIDUAL_BITS>();
    u_reader.reset(new UnalignedFileReader());
    _disk_index = new pipeann::SSDIndex<T, TagT>(this->_dist_metric, reader, u_reader, nbr_handler, true, &_paras_disk);
#else
    AbstractNeighbor<T> *nbr_handler = new PQNeighbor<T>();
    _disk_index = new pipeann::SSDIndex<T, TagT>(this->_dist_metric, reader, nbr_handler, true, &_paras_disk);
#endif
    ...
    bool use_page_search = (search_mode == PAGE_SEARCH);
#ifdef PMEMANN
    int res = _disk_index->load(_disk_index_prefix_in.c_str(), _pmem_index_prefix_in.c_str(), _num_threads, true, use_page_search);
#else
    int res = _disk_index->load(_disk_index_prefix_in.c_str(), _num_threads, true, use_page_search);
#endif
    if (res != 0) {
...
#ifdef PMEMANN
    return _disk_index->insert_in_place_pmemann(point, tag, deletion_set);
#else
  ...
  void DynamicSSDIndex<T, TagT>::merge(const uint32_t &nthreads, const uint32_t &n_sampled_nbrs) {
    ...
    // _disk_index->merge_deletes(deletion_sets[1 - active_delete_set], nthreads, n_sampled_nbrs);
  }
