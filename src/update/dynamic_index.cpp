...
namespace dirann {
  ...
  DynamicSSDIndex<T, TagT>::DynamicSSDIndex(Parameters &parameters, const std::string disk_prefix_in,
                                            ...
                                            dirann::Metric dist_metric, int search_mode, bool use_mem_index, int strategy) {
    ...
    _disk_index = new dirann::SSDIndex<T, TagT>(this->_dist_metric, reader, false, true, &_paras_disk);
  ...
  void DynamicSSDIndex<T, TagT>::trigger_deletion(const uint32_t &nthreads, const uint32_t &n_sampled_nbrs) {
    ...
    save_del_set();
    dirann::Timer timer;
    // _disk_index->trigger_deletion_in_place2(deletion_sets[1 - active_delete_set], nthreads, n_sampled_nbrs);
    _disk_index->trigger_deletion_in_place3(deletion_sets[1 - active_delete_set], nthreads, n_sampled_nbrs);
    // _disk_index->trigger_deletion_in_place4(deletion_sets[1 - active_delete_set], nthreads, n_sampled_nbrs);
