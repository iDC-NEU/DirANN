...
namespace dirann {
  ...
  Index<T, TagT>::Index(Metric m, const size_t dim, const size_t max_points, const bool dynamic_index,
    ...
    LOG(INFO) << "Getting distance function for metric: " << (m == dirann::Metric::COSINE ? "cosine" : "l2");
  ...
  void Index<T, TagT>::save(const char *filename) {
    ...
    if (!_save_as_one_file) {
      ...
      dirann::save_bin<_u64>(filename, cumul_bytes.data(), cumul_bytes.size(), 1, 0);
  ...
  size_t Index<T, TagT>::load_data(std::string filename, size_t offset) {
    ...
    size_t file_dim, file_num_points;
    dirann::get_bin_metadata(filename, file_num_points, file_dim, offset);
  ...
  void Index<T, TagT>::load(const char *filename) {
    ...
    if (!_save_as_one_file) {
      ...
      dirann::load_bin<_u64>(index_file, file_offset_data, nr, nc, 0);
  ...
  void Index<T, TagT>::load_from_disk_index(const std::string &filename) {
    ...
    char *buf;
    dirann::alloc_aligned((void **) &buf, kSectorsPerRead * kSectorLen, kSectorLen);
...
#pragma omp parallel for schedule(static, 65536)
      ...
      if (best_L_nodes[k].flag) {
        ...
        for (unsigned m = 0; m < des.size(); ++m) {
          ...
          if (inserted_into_pool.find(id) == inserted_into_pool.end()) {
            ...
            if ((m + 1) < des.size()) {
              auto nextn = des[m + 1];
              dirann::prefetch_vector((const char *) _data + _aligned_dim * (size_t) nextn, sizeof(T) * _aligned_dim);
            }
  ...
  template<typename T, typename TagT>
  void Index<T, TagT>::DGAI_link(Parameters &parameters) {
...
#pragma omp parallel for schedule(dynamic, 65536)
  ...
  void Index<T, TagT>::build(const char *filename, const size_t num_points_to_load, Parameters &parameters,
    ...
    if (filename == nullptr) {
      ...
      dirann::get_bin_metadata(filename, file_num_points, file_dim);
      ...
      if (_enable_tags) {
        if (tag_filename == nullptr) {
          ...
          if (file_exists(tag_filename)) {
            ...
            size_t npts, ndim;
            dirann::load_bin(tag_filename, tag_data, npts, ndim);
