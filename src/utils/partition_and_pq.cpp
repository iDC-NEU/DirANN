...
#pragma omp parallel for schedule(static, 65536)
  ...
  cumul_bytes[1] = cumul_bytes[0] + dirann::save_bin<float>(pq_pivots_path.c_str(), full_pivot_data.get(),
                                                             (size_t) num_centers, dim, cumul_bytes[0]);
  cumul_bytes[2] = cumul_bytes[1] +
                   dirann::save_bin<float>(pq_pivots_path.c_str(), centroid.get(), (size_t) dim, 1, cumul_bytes[1]);
  cumul_bytes[3] = cumul_bytes[2] + dirann::save_bin<uint32_t>(pq_pivots_path.c_str(), rearrangement.data(),
                                                                rearrangement.size(), 1, cumul_bytes[2]);
  cumul_bytes[4] = cumul_bytes[3] + dirann::save_bin<uint32_t>(pq_pivots_path.c_str(), chunk_offsets.data(),
                                                                chunk_offsets.size(), 1, cumul_bytes[3]);
  dirann::save_bin<_u64>(pq_pivots_path.c_str(), cumul_bytes.data(), cumul_bytes.size(), 1, 0);
...
int generate_train_data(const std::string data_file, float *passed_train_data, size_t num_train, unsigned num_centers, unsigned num_pq_chunks, std::string pq_pivots_path) {
  ...
  if (!file_exists(pq_pivots_path)) {
    ...
    dirann::load_bin<_u64>(pq_pivots_path.c_str(), file_offset_data, nr, nc, 0);
    ...
    dirann::load_bin<float>(pq_pivots_path.c_str(), full_pivot_data, nr, nc, file_offset_data[0]);
    ...
    dirann::load_bin<float>(pq_pivots_path.c_str(), centroid, nr, nc, file_offset_data[1]);
    ...
    dirann::load_bin<uint32_t>(pq_pivots_path.c_str(), rearrangement, nr, nc, file_offset_data[2]);
    ...
    dirann::load_bin<uint32_t>(pq_pivots_path.c_str(), chunk_offsets, nr, nc, file_offset_data[3]);
  ...
  dirann::convert_types<T, float>(all_data.get(), all_data_tmp.get(), num_points, dim);
  // LOG(ERROR) << "1";
...
int generate_pq_data_from_pivots(const std::string data_file, unsigned num_centers, unsigned num_pq_chunks,
  ...
  for (size_t block = 0; block < num_blocks; block++) {
    ...
    dirann::convert_types<T, float>(block_data_T.get(), block_data_tmp.get(), cur_blk_size, dim);
...
#pragma omp parallel for schedule(static, 8192)
    ...
    if (num_centers > 256) {
      ...
      dirann::convert_types<uint32_t, uint8_t>(block_compressed_base.get(), pVec.get(), cur_blk_size, num_pq_chunks);
...
int estimate_cluster_sizes(const std::string data_file, float *pivots, const size_t num_centers, const size_t dim,
  ...
  dirann::get_bin_metadata(data_file, num_points, num_dim);
  size_t block_size = num_points <= BLOCK_SIZE ? num_points : BLOCK_SIZE;
...
int shard_data_into_clusters(const std::string data_file, float *pivots, const size_t num_centers, const size_t dim,
  ...
  for (size_t block = 0; block < num_blocks; block++) {
    ...
    base_reader.read((char *) block_data_T.get(), sizeof(T) * (cur_blk_size * dim));
    dirann::convert_types<T, float>(block_data_T.get(), block_data_float.get(), cur_blk_size, dim);
...
int partition_with_ram_budget(const std::string data_file, const double sampling_rate, double ram_budget,
  ...
  while (!fit_in_ram) {
    ...
    for (auto &p : cluster_sizes) {
      double cur_shard_ram_estimate = dirann::estimate_ram_usage(p, train_dim, sizeof(T), graph_degree);
  ...
  dirann::save_bin<float>(output_file.c_str(), pivot_data, (size_t) num_parts, train_dim);
