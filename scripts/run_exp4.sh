...
do
    ...
    if [ "$sys" = "greatorp" ]; then
        flag="-DBG_IO_THREAD -DINPLACE_DELETE -DREVERSE_GRAPH"
        search_mode=0 
        pipeline_width=4
        strategy=0
    else 
    ...
    # if [ "$mode" = "1" ]; then
    #     flag=${flag}" -DDONT_USE_OPT1 -DDONT_USE_OPT2"
    # elif [ "$mode" = "2" ]; then
    #     flag=${flag}" -DDONT_USE_OPT2"
    # fi
    ...
    do
        ...
        log_file_path=${log_path}/${sys_name}_${dataset_name}_${mode}.log
